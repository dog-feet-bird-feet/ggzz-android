...
sealed class ResultIndicator(
    ...
    class Similarity(score: Float) : ResultIndicator(
        ...
        percentage = score,
        description = when {
            score >= 70 -> "두 필기체의 전체적인 형태가 유사합니다."
            score >= 40 -> "두 필기체에 일부 유사한 특징이 있습니다."
    ...
    class Pressure(score: Float) : ResultIndicator(
        ...
        percentage = score,
        description = when {
            score >= 70 -> "두 필기체의 필압 강도가 유사합니다."
            score >= 40 -> "두 필기체의 필압 경향이 일부 비슷합니다."
    ...
    class Inclination(score: Float) : ResultIndicator(
        ...
        percentage = score,
        description = when {
            score >= 70 -> "두 필기체의 기울기 방향이 유사합니다."
            score >= 40 -> "두 필기체의 기울기 패턴이 일부 비슷합니다."
